...
Ngrams::Ngrams(int ngramSz, WordList & wl)
{
   ...
   wl.beginIter(); //start to read the file
   while (!wl.endIter()) //while the file is not at the end do this code
   {
      std::string ngram = wl.getNextNgram(ngramSz);  //put all the words (size) into the string ngram
      wl.incrementIter(); //increment to the next one?
      //insert the ngram in the linked list
      if (!ngram.empty()) insertNgram(ngram); //if the string is not empty do the method insertNgram
   }
}
